...
public sealed class TagsController : NnaController
{
    ...
    {
        var tagDescription = await _tagsRepository.GetTagDescriptionAsync(Guid.Parse(id),  cancellationToken);
        return OkWithData(new TagDescriptionDto { Description = tagDescription });
    }
}